...
class StepResult:
    def __init__(self, step_id: str, status: StepStatus, output: Any = None, error: str | None = None):
...
class Executor:
    ...
    async def run(self, plan: Plan) -> list[StepResult]:
        ...
        for step in plan.steps:
            ...
            for dep_id in step.depends_on:
                ...
                if dep_status != StepStatus.DONE:
                    result = StepResult(step.step_id, StepStatus.SKIPPED,
            ...
            if result.status == StepStatus.FAILED:
                await event_bus.emit(Events.STEP_FAILED, {
        ...
        await event_bus.emit(Events.TASK_DONE, {
            "session_id": self.session.session_id,
    ...
    async def _run_step(self, step: PlanStep) -> StepResult:
        ...
        await event_bus.emit(Events.STEP_STARTED, {
            "session_id": self.session.session_id,
            "step_id": step.step_id,
            "title": step.title,
        })
        ...
        try:
            ...
            await event_bus.emit(Events.STEP_DONE, {
                "session_id": self.session.session_id,
                "step_id": step.step_id,
