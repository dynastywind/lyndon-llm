...
LANGUAGES: dict[str, LangSpec] = {
    ...
    "kotlin": LangSpec(
        ...
        local_bins=["kotlinc"],
        local_shell_cmd=(
            "cd {tmpdir} && kotlinc {file} -include-runtime -d main.jar && java -jar main.jar"
        ),
    ),
    "scala": LangSpec(
        label="Scala",
    ...
    "objc": LangSpec(
        ...
        shell_cmd=(
            "clang -x objective-c /code/main.m -framework Foundation -o /tmp/main 2>&1 && /tmp/main"
        ),
        local_bins=["clang"],
        local_shell_cmd=("clang -x objective-c {file} -framework Foundation -o {out} && {out}"),
    ),
}
...
async def run_code(
    ...
    if spec is None:
        return _err(
            f"Unsupported language: {language!r}.  Supported: {', '.join(sorted(LANGUAGES))}"
        )
...
async def _run_docker(spec: LangSpec, code: str, timeout: int) -> dict:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if spec.shell_cmd:
            ...
            entrypoint = [c.replace("{file}", f"/code/{fname}") for c in spec.cmd]
        ...
        docker_cmd = [
            ...
            "--read-only",
            "--tmpfs",
            "/tmp:size=128m",  # 128m for compiled artefacts
            "-v",
            f"{tmpdir}:/code:ro",
            spec.docker_image,
            *entrypoint,
...
async def _run_process(spec: LangSpec, code: str, timeout: int) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if spec.local_shell_cmd:
            ...
            cmd_str = (
                spec.local_shell_cmd.replace("{file}", fpath)
                .replace("{out}", out)
