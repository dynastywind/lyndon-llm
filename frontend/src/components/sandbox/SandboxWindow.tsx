...
function OutputPanel({
  ...
  return (
    <div style={{
      ...
      borderLeft: '1px solid var(--lv-rule)', minWidth: 0,
      background: '#0d0d0d',
    }}>
      {/* Output header */}
      <div style={{
        ...
        <span style={{
          ...
          textTransform: 'uppercase', color: 'var(--lv-mute)', fontWeight: 500, flex: 1,
        }}>Output</span>
        ...
        {result && (
          <>
            ...
            {/* Copy */}
            {hasOutput && (
              <button onClick={handleCopy} title="Copy output" style={{
                background: 'none', border: 'none', cursor: 'pointer',
                color: 'var(--lv-mute)', padding: 2, lineHeight: 0,
              }}
      ...
      <div style={{
        ...
        {running && (
          <p style={{ color: 'var(--lv-mute)', fontSize: 11 }}>Running…</p>
        )}
        {result && (
          <>
            {result.stdout && (
              <pre style={{
                color: '#e8e3d8', margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word',
              }}>
                {result.stdout}
              </pre>
            )}
            {result.stderr && (
              <pre style={{
                color: '#e06c75', margin: 0, marginTop: result.stdout ? 12 : 0,
            ...
            {!result.stdout && !result.stderr && (
              <p style={{ color: 'var(--lv-mute)', fontSize: 11 }}>
                (no output)
              </p>
            )}
          </>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: '10px 16px', borderBottom: '1px solid var(--lv-rule)',
        flexShrink: 0,
      }}>
        {/* Language tabs */}
        <div style={{ display: 'flex', gap: 2 }}>
          {(languages.length > 0 ? languages : Object.keys(DEFAULTS).map(id => ({ id, label: id, available: true, runtime: 'process' as const }))).map((lang) => (
            <button
              key={lang.id}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <select
            ...
            style={{
              ...
              color: 'var(--lv-soft)', fontFamily: 'var(--font-mono)', fontSize: 10,
        ...
        <button
          ...
          style={{
            background: 'none', border: 'none', cursor: 'pointer',
            color: 'var(--lv-mute)', padding: 4, lineHeight: 0,
          }}
          className="hover:!text-[var(--lv-ink)] transition-colors"
        >
          <Trash2 size={13} />
